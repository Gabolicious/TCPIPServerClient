...
using static Discord2Server.Server;

namespace Discord2Server
{
    public partial class frmServer : Form
    {
        ...
        private delegate void UpdateChatRoomDelegate(int ClientNumber, int Chatroom);
        private void UpdateChatRoom(int ClientNumber, int Chatroom)
        {
            if (InvokeRequired)
            {
                Invoke(new UpdateChatRoomDelegate(UpdateChatRoom), new object[] { ClientNumber, Chatroom });
                return;
            }
            listView1.Items[ClientNumber.ToString()].SubItems[6].Text = Chatroom.ToString();
        }
        ...
        private void SetNewConnectionData_FromThread(int clientNumber)
        {
            ...
            try
            {
                lock (svr.workerSockets)
                {
                    ...
                    li.SubItems.Add("0");                  //index 6 Chat
                    /*******************************************************************************************************/
                }
            ...
            try
            {
                ...
                lock (svr.workerSockets)
                {
                    ...
                    if (svr.workerSockets.ContainsKey(clientNumber))
                    {
                        ...
                        ClientsName.CopyTo(0, xdata.szStringDataA, 0, ClientsName.Length);
                        xdata.Data10 = clientNumber;

                        byte[] byData = PACKET_FUNCTIONS.StructureToByteArray(xdata);

                        foreach (KeyValuePair<int, UserSock> user in svr.workerSockets)
                        {
                            if (user.Key != clientNumber)
                            {
                                svr.SendMessage(user.Key, byData); //WORKING ON THIS
                                PACKET_DATA ydata = new PACKET_DATA();
                                ydata.Packet_Type = (UInt16)PACKETTYPES.TYPE_ClientData;
                                user.Value.szClientName.CopyTo(0, ydata.szStringDataA, 0, user.Value.szClientName.Length);
                                ydata.Data10 = user.Key;
                                byte[] bxData = PACKET_FUNCTIONS.StructureToByteArray(ydata);

                                svr.SendMessage(clientNumber, bxData);
                            }
                        }
                    ...
                    while (FullPacketList.Count > 0)
                    {
                        ...
                        switch (type)//Interrigate the first 2 Bytes to see what the packet TYPE is
                        {
                            ...
                            case (UInt16)PACKETTYPES.TYPE_Message:
                                ...
                                break;
                            case (UInt16)PACKETTYPES.TYPE_ChangeChatRoom:
                                MoveChatRoom(fp.ThePacket, fp.iFromClient);
                                break;
                            default:
        ...
        private void MoveChatRoom(byte[] message, int FirstClientID)
        {
            PACKET_DATA IncomingData = new PACKET_DATA();
            IncomingData = (PACKET_DATA)PACKET_FUNCTIONS.ByteArrayToStructure(message, typeof(PACKET_DATA));
            int SecondClientID = IncomingData.Data10;
            PACKET_DATA xdata = new PACKET_DATA();
            xdata.Packet_Type = (UInt16)PACKETTYPES.TYPE_ChangeChatRoom;
            if (SecondClientID != 0)
            {
                svr.workerSockets[SecondClientID].szClientName.CopyTo(0, xdata.szStringDataA, 0, svr.workerSockets[SecondClientID].szClientName.Length);
            }
            //xdata.Data10 = FirstClientID;
            //svr.workerSockets[FirstClientID].szClientName.CopyTo(0, xdata.szStringDataA, 0, svr.workerSockets[FirstClientID].szClientName.Length);
            svr.workerSockets[FirstClientID].ChatRoom = SecondClientID;
            /*
            if (SecondClientID != 0)
            {
                svr.workerSockets[SecondClientID].ChatRoom = FirstClientID;
                byte[] yData = PACKET_FUNCTIONS.StructureToByteArray(xdata);
                svr.SendMessage(SecondClientID, yData);
                xdata.szStringDataA = new Char[300];
                svr.workerSockets[SecondClientID].szClientName.CopyTo(0, xdata.szStringDataA, 0, svr.workerSockets[SecondClientID].szClientName.Length);
                UpdateChatRoom(SecondClientID, FirstClientID);
            }
            */
            xdata.Data10 = SecondClientID;
            byte[] byData = PACKET_FUNCTIONS.StructureToByteArray(xdata);
            svr.SendMessage(FirstClientID, byData);
            UpdateChatRoom(FirstClientID, SecondClientID);
        }
                    ...
                    case (UInt16)PACKETTYPES_SUBMESSAGE.SUBMSG_MessageStart:
                        {
                            if (svr.workerSockets.ContainsKey(clientID))
                            {
                                ...
                                if (svr.workerSockets[clientID].ChatRoom == 0)
                                {
                                    svr.SendMessage(message);
                                ...
                                {
                                    svr.SendMessage(svr.workerSockets[clientID].ChatRoom, message);
                                    svr.SendMessage(clientID, message);
                                }
                    ...
                    case (UInt16)PACKETTYPES_SUBMESSAGE.SUBMSG_MessageGuts:
                        {
                            ...
                            if (svr.workerSockets[clientID].ChatRoom == 0)
                            {
                                foreach (KeyValuePair<int, UserSock> user in svr.workerSockets)
                                {
                                    svr.SendMessage(user.Key, message);
        ...
        private void SendMessageOfClientDisconnect(int clientId)
        {
            try
            {
                ...
                svr.workerSockets[clientId].szClientName.CopyTo(0, xdata.szStringDataA, 0, svr.workerSockets[clientId].szClientName.Length);
                byte[] byData = PACKET_FUNCTIONS.StructureToByteArray(xdata);
                svr.SendMessage(byData);
            }
            catch { }
        ...
        private void CommunicationsDisplay_DocumentCompleted(object sender, WebBrowserDocumentCompletedEventArgs e)
        {
